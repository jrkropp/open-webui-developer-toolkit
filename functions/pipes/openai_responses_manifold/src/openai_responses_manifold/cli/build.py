...
def _render_file_tree_comment() -> str:
    ...
    """
    lines: list[str] = [
        "# NOTE: This file is generated by openai_responses_manifold.cli.build.",
...
def _validate_module_order() -> None:
    ...
    raise RuntimeError(
        ...
        "Add them to cli/build.py (respecting the desired order) so they are bundled:\n"
        f"{missing_lines}"
    )
...
def _collect_internal_imports(source: str, lines: list[str]) -> list[InternalImportStmt]:
    """Parse the module AST and collect all import statements that refer to internal modules."""
    try:
        tree = ast.parse(source)
    except SyntaxError:
        return []
...
def register(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    parser = subparsers.add_parser("build", help="Bundle the pipe for Open WebUI.")
    parser.add_argument(
        "--skip-tests",
        action="store_true",
        help="Skip running pytest before building.",
