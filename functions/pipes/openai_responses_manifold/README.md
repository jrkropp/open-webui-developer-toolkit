...
| **Web search tool**                | ✅ GA            | 2025-06-03   | Injects OpenAI’s `web_search` tool into supported models. With the valve enabled, the model can **self-decide when to search**. Supports OpenAI’s `allowed_domains`, `user_location`, `external_web_access`, and auto-requests `web_search_call.action.sources` so citations stay clickable.                                                                                                  |
...
| **Code interpreter**               | ✅ GA           | 2025-11-21   | Enables OpenAI’s built-in **code_interpreter** tool. Turn it on via valve `ENABLE_CODE_INTERPRETER_TOOL=true` (optional container override: `CODE_INTERPRETER_CONTAINER_JSON`). Auto-includes `code_interpreter_call.outputs` and streams status; citations include executed code + logs.                     |
...
- **Outbound requests** use `adapters/openai/requests.py` (`ResponseCreateParams`) with `extra="forbid"`; the pipe uses `adapters/openwebui/request_builder.py` to adapt OpenWebUI chat payloads into the strict Responses API shape.
